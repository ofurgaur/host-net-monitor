...
const map=L.map('map',{worldCopyJump:true}).setView([25,0],2);const lines=L.layerGroup().addTo(map);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors',maxZoom:18}).addTo(map);const layer=L.layerGroup().addTo(map);let mode='shade';
...
def aggregate(rows: list[dict]) -> dict:
    cities: dict[tuple[str, str], dict] = defaultdict(lambda: {"flow_count": 0, "bytes": 0, "latitude": None, "longitude": None, "last_seen": ""})
    for item in rows:
        geo = item.get("geo") or {}
        ...
        if timestamp > out["last_seen"]:
            out["last_seen"] = timestamp
    city_list = []
    ...
    return {"updated": latest, "line_epoch": time.time(), "total_flow_count": sum(x["flow_count"] for x in city_list), "total_bytes": sum(x["bytes"] for x in city_list), "cities": city_list, "rows": row_items}
    ...
    def do_POST(self):
        ...
        try:
            ...
            for line in raw.splitlines():
                ...
                accepted += 1
            if accepted:
                snapshot = aggregate(read_stream(self.server.redis, self.server.stream, self.server.limit))
                keys = set()
                for event in events:
                    geo = event.get("geo") or {}
                    city = geo.get("city_name") or geo.get("city") or "Unknown city"
                    country = geo.get("country_name") or geo.get("country_code") or ""
                    keys.add(f"{city}|{country}")
                snapshot["new_line_keys"] = sorted(keys)
                self.server.broadcaster.publish(snapshot)
